...
test -f /etc/resolv.conf && echo "/etc/resolv.conf ia a regular file" || echo "/etc/resolv.conf is not a regular file"
...
[ /etc/hosts -nt /etc/resolv.conf ] && echo "/etc/hosts is the newest file" || echo "/etc/resolv.conf is the newest file"
...
read -p "$fnum" firstNumber
snum="Enter second number: "
read -p "$snum" secondNumber
...
[[ $((secondNumber % firstNumber)) -eq 0 ]] && echo "The second number is multiple of the first number" || echo "The second number is not the multiple of the first number"
...
USER="today" && [ -v USER ] && [ -n "$USER" ] && echo "The variable has data in it and it is not empty"
USER= && [ -v USER ] && [ -z "$USER" ] && echo "The variable is empty"
...
#b=01
fstring="Enter first string: "
read -p "$fstring" a
sstring="Enter second string: "
read -p "$sstring" b
