...
#to show the list of interface names
#interfaceName=$(ip a |awk '/: e/{gsub(/:/,"");print $2}')
 for (( interfaceName=0; interfaceName < 5; interfaceName++ )); do
   interfaceName=$(ip a |awk '/: e/{gsub(/:/,"");print $2}')
...
List of interface names : $interfaceName
EOF
